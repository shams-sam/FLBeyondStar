...
for (idx, history), n in zip(enumerate(args.baselines), ['c', 'f']):
    ...
    break
ax2.hlines(y=power['c']/k2, xmin=0.5, xmax=4.5, color='k', label=r'FL,$\tau$=1 (full)')
...
if len(args.baselines) > 1:
    ...
    ax2.hlines(y=power['f']/k2, xmin=0.5, xmax=4.5, color='r', label=r'FL,$\tau$=20 (sampled)')
